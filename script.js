window.btns = []
var template = `game:GetService("StarterGui"):SetCore("SendNotification",{     
Title = "CREDITS",     
Text = "This script is hosted by ROOT44x.",
Button1 = "Ok.",
Duration = 7
})
...
$(document).ready(function() {
    $("#output").hide()
    $("#Generatehub").click(function() {
        window.code = template.replace("{name}", window.hubname)
        for (var i in btns) {
            window.code += `

Main:Button("${btns[i][0]}",function()
${btns[i][1]}
end)`
        }

        for (var i in labels) {
            if (labels[i][0] != null || labels[i][0] != undefined) { // A Label worth putting time into.
                window.code += `
Main:Label("${labels[i][0]}")
`
            }
        }
        console.log(window.code)
        $("#output").val(window.code)
        $("#output").show();
    })
})
